...
def download_flv(start, size, aid, url, pid, qn, part):
    global process
    header = {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.13; rv:56.0) Gecko/20100101 Firefox/56.0',
        'Accept': '*/*',
    ...
    with open("av" + str(aid) + "_p" + str(pid) + "_%d" % qn + "_part" + str(part) + ".flv", "ab+") as flv:
...
def main():
    ...
    print("# 合并文件中...")
    file1 = open("av%s_p%d_%d_part1.flv" % (avid, av_info['pid'], av_info['qn']), "rb")
    file2 = open("av%s_p%d_%d_part2.flv" % (avid, av_info['pid'], av_info['qn']), "rb")
    file3 = open("av%s_p%d_%d_part3.flv" % (avid, av_info['pid'], av_info['qn']), "rb")
    file4 = open("av%s_p%d_%d_part4.flv" % (avid, av_info['pid'], av_info['qn']), "rb")

    file_all = open("av%s_p%d_%d.flv" % (avid, av_info['pid'], av_info['qn']), "wb")
    ...
    file1.close()
    os.remove("av%s_p%d_%d_part1.flv" % (avid, av_info['pid'], av_info['qn']))
    ...
    while 1:
        ...
        file_all.write(file_bytes)
    file2.close()
    os.remove("av%s_p%d_%d_part2.flv" % (avid, av_info['pid'], av_info['qn']))
    ...
    while 1:
        ...
        file_all.write(file_bytes)
    file3.close()
    os.remove("av%s_p%d_%d_part3.flv" % (avid, av_info['pid'], av_info['qn']))
    ...
    while 1:
        ...
        file_all.write(file_bytes)
    file4.close()
    os.remove("av%s_p%d_%d_part4.flv" % (avid, av_info['pid'], av_info['qn']))
    file_all.close()
